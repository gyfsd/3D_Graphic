...
class render_{
	...
	void draw_point(point_ point,cam camd,SDL_Surface *scr){
		...
		d.circle((int)pointd.x,(int)pointd.y,3,0xffffffff,scr);}
	}
	...
	mesh create_sphere(float x,float y,float z,float r,point_ *p,uint32_t *edges,edge_ *edgesS){
		...
		while(fy < 360){
			...
			while(fx < 360){
				...
				if(pC % 2 == 1){
				sphere.edges[sphere.edgesC].color = 0xff;
				}
...
class main_{
	...
	int main(){
		...
		//===============================================================================
		render.render(cube,c,edgesbuf,edgesbufC,zlm,sdl.scr);
		//d.blur_scr(5,sdl.scr,blurmem);
		sdl.flip();
		d.fill(0,sdl.scr);
		//=================================================================================
		keyd = event.getkey();
